...
EVP_CIPHER_CTX* real_EVP_CIPHER_CTX_new(void)
{
    ...
    if (NULL == EVP_CIPHER_CTX_new_func_ptr)
        EVP_CIPHER_CTX_new_func_ptr = (EVP_CIPHER_CTX_new_func_t)dlsym(RTLD_NEXT, "EVP_CIPHER_CTX_new");
    if (NULL != EVP_CIPHER_CTX_new_func_ptr)
...
EVP_CIPHER_CTX* EVP_CIPHER_CTX_new(void)
{
    ...
    if (esan_should_I_fail()) {
        ...
        obj_stats[ESAN_EVP_CIPHER_CTX_NEW].esan_nr_failed_executions += 1;
...
EVP_MD_CTX* real_EVP_MD_CTX_create(void)
{
    ...
    if (NULL == EVP_MD_CTX_create_func_ptr)
        EVP_MD_CTX_create_func_ptr = (EVP_MD_CTX_create_func_t)dlsym(RTLD_NEXT, "EVP_MD_CTX_create");
    if (NULL != EVP_MD_CTX_create_func_ptr)
...
EVP_MD_CTX* EVP_MD_CTX_create(void)
{
    ...
    if (esan_should_I_fail()) {
        ...
        obj_stats[ESAN_EVP_MD_CTX_CREATE].esan_nr_failed_executions += 1;
